...
async function Login(req, res) {  
  ...
  try {
    ...
    try {
      ...
      // If we get here, user exists, so continue with login attempt
    ...
    try {
      ...
      // This is likely a password error since we already confirmed user exists
      logger.error(`Login error for existing user: ${loginError}`);
      return res.status(400).send({ 
